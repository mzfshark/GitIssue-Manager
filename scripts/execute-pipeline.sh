...
# NEW: Supports CLI arguments for selective execution
###############################################################################
...
show_help() {
    cat << 'HELP'
GitIssue-Manager: 7-Stage Execution Pipeline (v3+)

USAGE:
  bash scripts/execute-pipeline.sh [OPTIONS]

OPTIONS:
  --help, -h                Show this help message
  --stage <NUM>             Execute only specific stage (1-7)
  --repo <REPO>             Execute only for specific repo:
                            - AragonOSX
                            - aragon-app
                            - Aragon-app-backend
...
EXAMPLES:
  ...
  # Run only STAGE 3 (CREATE parent and child issues)
  bash scripts/execute-pipeline.sh --stage 3

  # Run only for AragonOSX repo
  bash scripts/execute-pipeline.sh --repo AragonOSX

  # Run STAGE 3 only for aragon-app
  bash scripts/execute-pipeline.sh --stage 3 --repo aragon-app

  # List all stages
  bash scripts/execute-pipeline.sh --list-stages

STAGES:
  1: SETUP               - Verify auth, repos, configuration
  2: PREPARE             - Parse PLAN.md files, generate config
  3: CREATE              - Create parent + child issues with auto-linking
  4: FETCH               - Verify created issues
  5: APPLY METADATA      - Prepare ProjectV2 sync
  6: REPORTS             - Generate audit logs
...
Available Stages:
  ...
  5: APPLY METADATA      - Prepare ProjectV2 sync
  6: REPORTS             - Generate audit logs
  7: HIERARCHY           - Build issue hierarchy + progress tracking (NEW!)
STAGES
}
...
parse_arguments() {
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --stage)
                EXEC_STAGE="$2"
                if ! [[ "$EXEC_STAGE" =~ ^[1-7]$ ]]; then
                    error "Invalid stage: $EXEC_STAGE (must be 1-7)"
                fi
                shift 2
                ;;
            --repo)
                EXEC_REPO="$2"
                case "$EXEC_REPO" in
                    AragonOSX|aragon-app|Aragon-app-backend)
                        shift 2
                        ;;
                    *)
                        error "Invalid repo: $EXEC_REPO"
                        ;;
                esac
                ;;
            --list-stages)
...
should_exec_stage() {
    local stage_num=$1
    if [ -z "$EXEC_STAGE" ]; then
        return 0  # Execute if no specific stage requested
    fi
    [ "$stage_num" = "$EXEC_STAGE" ]
}

should_exec_repo() {
    local repo_name=$1
    if [ -z "$EXEC_REPO" ]; then
        return 0  # Execute if no specific repo requested
    fi
    [ "$repo_name" = "$EXEC_REPO" ]
}
...
stage_create() {
    ...
    # Issue 1: AragonOSX
    if should_exec_repo "AragonOSX"; then
...
- [ ] Test native-token voting scenarios"
    ...
    fi  # End AragonOSX block

    # Issue 2: aragon-app
    if should_exec_repo "aragon-app"; then
...
- [ ] Test governance permission checks"
    ...
    fi  # End aragon-app block

    # Issue 3: Aragon-app-backend
    if should_exec_repo "Aragon-app-backend"; then
...
- [ ] Monitoring and alerting validation post-deploy"
    ...
    fi  # End Aragon-app-backend block
...
###############################################################################
# STAGE 7: BUILD HIERARCHY & PROGRESS TRACKING
###############################################################################

stage_hierarchy() {
    log "════════════════════════════════════════════════════════════════"
    log "STAGE 7: HIERARCHY (Build issue hierarchy + progress tracking)"
    log "════════════════════════════════════════════════════════════════"

    # Process hierarchy for each repository
    info "Processing hierarchy from .md files..."

    if should_exec_repo "AragonOSX"; then
        local repo_path="/mnt/d/Rede/Github/mzfshark/AragonOSX"
        info "Processing: Axodus/AragonOSX"
        
        if [[ -f "$repo_path/PLAN.md" ]]; then
            node "$PROJECT_ROOT/scripts/process-hierarchy.js" "$repo_path" "Axodus/AragonOSX" 2>&1 | tee -a "${AUDIT_LOG}"
            success "Generated hierarchy for AragonOSX"
        else
            warning "PLAN.md not found in AragonOSX"
        fi
    fi

    if should_exec_repo "aragon-app"; then
        local repo_path="/mnt/d/Rede/Github/mzfshark/aragon-app"
        info "Processing: Axodus/aragon-app"
        
        if [[ -f "$repo_path/PLAN.md" ]]; then
            node "$PROJECT_ROOT/scripts/process-hierarchy.js" "$repo_path" "Axodus/aragon-app" 2>&1 | tee -a "${AUDIT_LOG}"
            success "Generated hierarchy for aragon-app"
        else
            warning "PLAN.md not found in aragon-app"
        fi
    fi

    if should_exec_repo "Aragon-app-backend"; then
        local repo_path="/mnt/d/Rede/Github/mzfshark/Aragon-app-backend"
        info "Processing: Axodus/Aragon-app-backend"
        
        if [[ -f "$repo_path/PLAN.md" ]]; then
            node "$PROJECT_ROOT/scripts/process-hierarchy.js" "$repo_path" "Axodus/Aragon-app-backend" 2>&1 | tee -a "${AUDIT_LOG}"
            success "Generated hierarchy for Aragon-app-backend"
        else
            warning "PLAN.md not found in Aragon-app-backend"
        fi
    fi

    success "STAGE 7 COMPLETE: Issue hierarchy processed"
}
...
main() {
    ...
    # Show execution mode
    if [ -n "$EXEC_STAGE" ]; then
        info "Mode: Single Stage Execution (STAGE $EXEC_STAGE)"
    fi
    if [ -n "$EXEC_REPO" ]; then
        info "Mode: Single Repository Execution ($EXEC_REPO)"
    fi
    echo ""

    should_exec_stage 1 && stage_setup && echo ""
    should_exec_stage 2 && stage_prepare && echo ""
    should_exec_stage 3 && stage_create && echo ""
    should_exec_stage 4 && stage_fetch && echo ""
    should_exec_stage 5 && stage_apply_metadata && echo ""
    should_exec_stage 6 && stage_reports && echo ""
    should_exec_stage 7 && stage_hierarchy && echo ""
